...
void tampilkanJadwal(const char *databaseFile, const char *maskapai) {
    ...
    fclose(dataFile);
    printf("\n\nJika sudah menentukan jadwal yang akan diedit,\nIngat kode jadwal dan Klik ENTER untuk input :] ");
    if (i == 1) {
...
void editJadwal(const char *databaseFile,  const char *maskapai) {
    ...
    while (fgets(line, sizeof(line), dataFile)) {
        if (strstr(line, kode) != NULL && strstr(line, maskapai) != NULL) {
            ditemukan = 1;
            printf("Jadwal yang dipilih : \n\n%s\n\n", line);
            
            informasiPenerbangan jadwalBaru;

            printf("Lengkapi Data untuk Jadwal Baru\n");
            printf("Hari : ");
    ...
    if (ditemukan) {
        ...
        printf("Jadwal Penerbangan berhasil diedit.\n");
    } else {
...
void tambahJadwal(const char *databaseFile) {
    ...
    fclose(dataFile);
    printf("Jadwal Penerbangan berhasil ditambahkan.\n");
